...
public class ArticleController implements Initializable {
    ...
    private void loadSpecialites() {
        ...
        } catch (SQLException e) { System.err.println("Specialites: " + e.getMessage()); }
    }
    private String getSpecialiteNom(int id) { return specialiteMap.getOrDefault(id, "General"); }

    private void chargerFeed() throws SQLException {
        tousLesArticles = articleService.getAllByRole(isDoctor);
    ...
    private VBox buildPost(Article article) {
        ...
        avatar.setStyle("-fx-background-color:#eff6ff;-fx-background-radius:50%;-fx-font-size:13px;-fx-font-weight:bold;-fx-text-fill:#1d4ed8;-fx-min-width:46px;-fx-min-height:46px;-fx-max-width:46px;-fx-max-height:46px;-fx-alignment:center;");
        VBox authorInfo = new VBox(2);
        Label lblAuteur = new Label("Dr. Medecin");
        lblAuteur.setStyle("-fx-font-size:14px;-fx-font-weight:bold;-fx-text-fill:#0f172a;");
        String dateStr = article.getDatePublication() != null ? new SimpleDateFormat("dd MMM yyyy 'a' HH:mm").format(article.getDatePublication()) : "-";
        ...
        Label badge = new Label("publi".equals(statut) || "publié".equals(statut) ? "Publie" : "Brouillon");
        badge.setStyle("publi".equals(statut) || "publié".equals(statut)
        ...
        lblContenu.setStyle("-fx-font-size:13px;-fx-text-fill:#334155;");
        ...
        if (article.getImage() != null && !article.getImage().isBlank()) {
            try {
                javafx.scene.image.Image img = new javafx.scene.image.Image(article.getImage(), 680, 320, true, true, true);
                img.progressProperty().addListener((obs, ov, nv) -> {
                    if (nv.doubleValue() == 1.0 && !img.isError()) {
                        imageView.setImage(img); imageView.setFitWidth(680); imageView.setFitHeight(320);
                        imageView.setPreserveRatio(true); imageView.setVisible(true); imageView.setManaged(true);
                    }
                });
            } catch (Exception ex) { System.err.println("Image: " + ex.getMessage()); }
        }
        ...
        btnLike.setMaxWidth(Double.MAX_VALUE); HBox.setHgrow(btnLike, Priority.ALWAYS);
        HBox emojiPopup = buildEmojiPopup(article, btnLike, lblReactionSummary);
        emojiPopup.setVisible(false); emojiPopup.setManaged(false); emojiPopup.setTranslateY(-52);
        reactionContainer.getChildren().addAll(btnLike, emojiPopup);
        btnLike.setOnMouseEntered(e -> showPopup(emojiPopup));
        btnLike.setOnMouseExited(e -> { javafx.animation.PauseTransition p = new javafx.animation.PauseTransition(Duration.millis(300)); p.setOnFinished(ev -> { if (!emojiPopup.isHover()) hidePopup(emojiPopup); }); p.play(); });
        emojiPopup.setOnMouseExited(e -> hidePopup(emojiPopup));
        btnLike.setOnAction(e -> { int uid = getCurrentUserId(); if (uid == 0) return; try { reactionService.upsertReaction(article.getId(), uid, "LIKE"); rafraichirReactions(article, lblReactionSummary, btnLike); } catch (SQLException ex) { ex.printStackTrace(); } });
        ...
        btnCommenter.setMaxWidth(Double.MAX_VALUE); HBox.setHgrow(btnCommenter, Priority.ALWAYS);
        actionBar.getChildren().add(btnCommenter);

        Button btnTraduire = actionBarBtn("Traduire");
        btnTraduire.setMaxWidth(Double.MAX_VALUE); HBox.setHgrow(btnTraduire, Priority.ALWAYS);
        actionBar.getChildren().add(btnTraduire);

        if (isDoctor) {
            Button btnModifier = actionBarBtn("Modifier");
        ...
        btnTraduire.setOnAction(e -> { boolean v = !translatePanel.isVisible(); translatePanel.setVisible(v); translatePanel.setManaged(v); btnTraduire.setText(v ? "Fermer traduction" : "Traduire"); });
        ...
        txtComment.setPromptText("Ecrire un commentaire...");
        ...
        Runnable envoyerAction = () -> {
            ...
            if (analyse.rejected) {
                ...
                alert.setTitle("Commentaire rejete");
                alert.setHeaderText("Votre commentaire a ete bloque par la moderation IA");
                alert.setContentText(analyse.getSummary() + "\n\nRaison : " + analyse.rejectionReason);
                alert.showAndWait();
                return;
            }
            try {
                Commentaire c = new Commentaire();
                c.setContenu(texte); c.setDateCommentaire(new Date()); c.setNbLikes(0); c.setArticle(article);
    ...
    private HBox buildEmojiPopup(Article article, Button btnLike, Label lblReactionSummary) {
        ...
        String[][] reactions = {{"LIKE","J'aime","#dbeafe","#1d4ed8"},{"LOVE","J'adore","#fee2e2","#dc2626"},{"HAHA","Haha","#fef9c3","#ca8a04"},{"WOW","Wow","#f3e8ff","#7c3aed"},{"SAD","Triste","#f1f5f9","#475569"},{"ANGRY","Grrr","#ffedd5","#ea580c"}};
        for (String[] r : reactions) {
            String type=r[0], label=r[1], bg=r[2], fg=r[3];
            Button btn = new Button(label);
            btn.setStyle("-fx-background-color:"+bg+";-fx-text-fill:"+fg+";-fx-font-size:11px;-fx-font-weight:bold;-fx-background-radius:20;-fx-border-color:transparent;-fx-padding:6 10;-fx-cursor:hand;");
            btn.setOnMouseEntered(e -> btn.setStyle("-fx-background-color:"+fg+";-fx-text-fill:white;-fx-font-size:11px;-fx-font-weight:bold;-fx-background-radius:20;-fx-border-color:transparent;-fx-padding:6 10;-fx-cursor:hand;"));
            btn.setOnMouseExited(e -> btn.setStyle("-fx-background-color:"+bg+";-fx-text-fill:"+fg+";-fx-font-size:11px;-fx-font-weight:bold;-fx-background-radius:20;-fx-border-color:transparent;-fx-padding:6 10;-fx-cursor:hand;"));
            btn.setOnAction(e -> { int uid = getCurrentUserId(); if (uid == 0) return; try { reactionService.upsertReaction(article.getId(), uid, type); rafraichirReactions(article, lblReactionSummary, btnLike); hidePopup((HBox) btn.getParent()); } catch (SQLException ex) { ex.printStackTrace(); } });
    ...
    private void rafraichirReactions(Article article, Label lblSummary, Button btnLike) {
        try {
            ...
            int shown = 0;
            for (Map.Entry<String, Integer> e : counts.entrySet()) { if (shown++ >= 3) break; sb.append(ReactionService.toLabel(e.getKey()).substring(0,2)).append(" "); }
            if (total > 0) sb.append(" ").append(total);
            lblSummary.setText(sb.toString().trim());
            if (uid != 0) { String myReaction = reactionService.getUserReaction(article.getId(), uid); btnLike.setText(myReaction != null ? ReactionService.toLabel(myReaction) : "J'aime"); }
    ...
    private HBox buildCommentRow(Commentaire commentaire, Article article, VBox liste, Label lblNb) {
        HBox row = new HBox(10); row.setAlignment(Pos.TOP_LEFT); row.setPadding(new Insets(6,0,6,0));
        ...
        String dateStr = commentaire.getDateCommentaire() != null ? new SimpleDateFormat("dd MMM 'a' HH:mm").format(commentaire.getDateCommentaire()) : "";
        HBox bubbleHeader = new HBox(8); bubbleHeader.setAlignment(Pos.CENTER_LEFT);
        ...
        Label lblDate = new Label(dateStr); lblDate.setStyle("-fx-font-size:11px;-fx-text-fill:#94a3b8;");
        Region sp = new Region(); HBox.setHgrow(sp, Priority.ALWAYS);
        bubbleHeader.getChildren().addAll(lblUser, lblDate, sp);
        Label lblContenu = new Label(commentaire.getContenu()); lblContenu.setWrapText(true); lblContenu.setStyle("-fx-font-size:13px;-fx-text-fill:#1e293b;");
        int currentUserId = getCurrentUserId();
        if (currentUserId != 0 && currentUserId == commentaire.getUtilisateurId()) {
            Button btnEdit = new Button("Edit"); btnEdit.setStyle("-fx-background-color:transparent;-fx-text-fill:#3b82f6;-fx-font-size:11px;-fx-cursor:hand;-fx-border-color:transparent;-fx-padding:0 4;");
            Button btnDel = new Button("X"); btnDel.setStyle("-fx-background-color:transparent;-fx-text-fill:#ef4444;-fx-font-size:11px;-fx-cursor:hand;-fx-border-color:transparent;-fx-padding:0 4;");
            ...
            btnEdit.setOnAction(e -> {
                ...
                Button btnSave = new Button("Enregistrer"); btnSave.setStyle("-fx-background-color:#1d4ed8;-fx-text-fill:white;-fx-font-size:11px;-fx-font-weight:bold;-fx-background-radius:6;-fx-border-color:transparent;-fx-padding:5 12;-fx-cursor:hand;");
                Button btnCancel = new Button("Annuler"); btnCancel.setStyle("-fx-background-color:#f1f5f9;-fx-text-fill:#475569;-fx-font-size:11px;-fx-font-weight:bold;-fx-background-radius:6;-fx-border-color:#e2e8f0;-fx-padding:5 12;-fx-cursor:hand;");
        ...
        } else { bubble.getChildren().addAll(bubbleHeader, lblContenu); }
        row.getChildren().addAll(avatarC, bubble);
        return row;
    }

    private VBox buildTranslatePanel(Article article) {
        VBox panel = new VBox(12); panel.setPadding(new Insets(14,16,14,16));
        panel.setStyle("-fx-background-color:#f0f9ff;-fx-border-color:#bae6fd;-fx-border-width:1 0 1 0;");
        Label lblTitle = new Label("Traduction de l'article"); lblTitle.setStyle("-fx-font-size:13px;-fx-font-weight:bold;-fx-text-fill:#0369a1;");
        HBox langRow = new HBox(10); langRow.setAlignment(Pos.CENTER_LEFT);
        ComboBox<TranslationService.Langue> cbSource = new ComboBox<>(); cbSource.getItems().addAll(TranslationService.Langue.values()); cbSource.setValue(TranslationService.Langue.FRANCAIS);
        ComboBox<TranslationService.Langue> cbCible = new ComboBox<>(); cbCible.getItems().addAll(TranslationService.Langue.values()); cbCible.setValue(TranslationService.Langue.ANGLAIS);
        Button btnGo = new Button("Traduire"); btnGo.setStyle("-fx-background-color:#0369a1;-fx-text-fill:white;-fx-font-size:12px;-fx-font-weight:bold;-fx-background-radius:8;-fx-border-color:transparent;-fx-padding:7 16;-fx-cursor:hand;");
        langRow.getChildren().addAll(new Label("De :"), cbSource, new Label("Vers :"), cbCible, btnGo);
        Label lblLoading = new Label("Traduction en cours..."); lblLoading.setStyle("-fx-font-size:12px;-fx-text-fill:#64748b;"); lblLoading.setVisible(false); lblLoading.setManaged(false);
        ...
        Label lblResultText = new Label(); lblResultText.setWrapText(true); lblResultText.setStyle("-fx-font-size:13px;-fx-text-fill:#1e293b;-fx-background-color:white;-fx-background-radius:8;-fx-border-radius:8;-fx-border-color:#e2e8f0;-fx-border-width:1;-fx-padding:10 12;");
        ...
        btnGo.setOnAction(e -> {
            ...
            if (src == cible) { lblResultTitle.setText("Choisissez deux langues differentes"); resultBox.setVisible(true); resultBox.setManaged(true); return; }
    ...
    private String getUserInitial() {
        if (UserSession.getCurrentUser() != null && UserSession.getCurrentUser().getUsername() != null)
            return UserSession.getCurrentUser().getUsername().substring(0,1).toUpperCase();
    ...
    private int getCurrentUserId() {
        if (UserSession.getCurrentUser() != null) return UserSession.getCurrentUser().getId();
    ...
    public void supprimerArticle(Article article) {
        ...
        if (confirm.showAndWait().isPresent() && confirm.getResult() == ButtonType.OK) {
            try { articleService.supprimer(article.getId()); chargerFeed(); }
            catch (SQLException e) { new Alert(Alert.AlertType.ERROR, "Impossible de supprimer l'article.").show(); e.printStackTrace(); }
        }
    }
}
