    ...
    private void handleCreateAccount() {
        ...
        if (role != null && (role.equalsIgnoreCase("Doctor") ||
            ...
            try {
                ...
                // Send pending approval email in background
                final String doctorEmail = createdUser.getEmail();
                final String doctorUsername = createdUser.getUsername();
                new Thread(() -> {
                    try {
                        esprit.fx.utils.EmailService.sendPendingApprovalEmail(doctorEmail, doctorUsername);
                        System.out.println("[RegisterController] Email attente validation envoyé à : " + doctorEmail);
                    } catch (Exception ex) {
                        System.err.println("[RegisterController] Erreur envoi email médecin : " + ex.getMessage());
                    }
                }, "doctor-pending-email").start();
