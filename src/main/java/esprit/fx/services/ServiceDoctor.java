...
public class ServiceDoctor implements IService<Doctor> {
    ...
    private void sendApprovalEmail(int userId) {
        try {
            ...
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                EmailService.sendDoctorApprovedEmail(rs.getString("email"), rs.getString("username"));
            }
        } catch (Exception e) {
    ...
    private void sendRejectionEmail(int userId, String reason) {
        try {
            ...
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                EmailService.sendDoctorRejectedEmail(rs.getString("email"), rs.getString("username"), reason);
            }
        } catch (Exception e) {
