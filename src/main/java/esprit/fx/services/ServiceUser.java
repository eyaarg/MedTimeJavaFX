...
public class ServiceUser implements IService<User> {
    ...
    public User login(String identifier, String password) throws SQLException {
        ...
        while (rs.next()) {
            if (user == null) {
                ...
                // Check doctor pending approval — throw a descriptive exception so the
                // controller can show a clear message without needing to inspect flags.
                // We still build the user object so the controller can also check flags
                // if it prefers; the exception path is the primary signal.
                user = new User(
                ...
                // Load profile photo path from DB into the session user object
                String photoPath = rs.getString("profile_photo");
                if (photoPath != null && !photoPath.isBlank()) {
                    user.setProfilePhotoFile(photoPath);
                }
            }
        ...
        // Explicit check: doctor account pending admin approval
        boolean isDoctor = roles.stream().anyMatch(r -> r.getName() != null &&
                (r.getName().toUpperCase().contains("DOCTOR") ||
                 r.getName().toUpperCase().contains("PHYSICIAN") ||
                 r.getName().toUpperCase().contains("MEDECIN")));
        if (!user.isActive() && isDoctor) {
            throw new SQLException("PENDING_APPROVAL: Your account is pending admin approval. " +
                    "An administrator must review your file before you can access the platform.");
        }
    ...
    /**
     * Returns true if the account for the given email/username is locked
     * (is_active=false AND failed_attempts >= 5).
     * Used by LoginController to show the correct error message after a failed login.
     */
    public boolean isAccountLocked(String identifier) throws SQLException {
        PreparedStatement ps = conn().prepareStatement(
                "SELECT is_active, failed_attempts FROM users WHERE email=? OR username=?");
        ps.setString(1, identifier);
        ps.setString(2, identifier);
        ResultSet rs = ps.executeQuery();
        if (rs.next()) {
    ...
    /**
     * Updates the profile photo path in the DB for the given user.
     * Delegates to ServiceProfilePhoto internally but also available
     * directly on ServiceUser for convenience.
     */
    public void updateProfilePhoto(int userId, String photoPath) throws SQLException {
        PreparedStatement ps = conn().prepareStatement(
    ...
    /**
     * Returns all users that have a doctor/physician role.
     * Used by DisponibiliteController and other modules.
     */
    public List<User> getAllDoctors() throws SQLException {
        String req = "SELECT u.*, r.id as role_id, r.name as role_name FROM users u " +
                "JOIN user_roles ur ON u.id = ur.user_id " +
                "JOIN roles r ON ur.role_id = r.id " +
                "WHERE UPPER(r.name) IN ('ROLE_PHYSICIAN','PHYSICIAN','DOCTOR','ROLE_DOCTOR','MEDECIN') " +
                "ORDER BY u.id";
        java.util.LinkedHashMap<Integer, User> userMap = new java.util.LinkedHashMap<>();
